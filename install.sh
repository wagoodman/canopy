...
VERIFY_SIGN=false
# canopy ships install.sh + cosign signing from its very first release, so every published tag (>= v0.1.0)
# supports both signature verification and the -v flag. bump these only if the first release tag differs.
...
# try to re-fetch a per-tag script (it won't exist -> 404)
INSTALL_SH_SUPPORTED_VERSION=v0.1.0
